...
Fase 0 publicada como `v0.1.0`. A Fase 2 terminou em `v0.6.0`, seguida da home em `v0.6.1` e busca/perfis em `v0.7.0`. A Fase 4 avançou de `v0.8.0` a `v0.11.0`; a revisão pública em linguagem simples saiu em `v0.12.0` após CI aprovada no commit `27d239c`. Despesas e fiscais permanecem parciais.

Evidências atuais: 64 testes PHP com 407 asserções e todos os gates passaram. O MySQL contém 361 fornecedores, 1.275 contratos e 1.073 licitações. O smoke no Edge cobriu home, fontes, pessoas, contratos e detalhe em desktop e celular, sem rolagem horizontal; legendas de linguagem simples apareceram nas telas de dados.
