...
Fase 0 publicada como `v0.1.0`. A Fase 1 foi publicada nas versões `v0.2.0` a `v0.2.2`. A estrutura organizacional saiu em `v0.3.0`, a camada bruta KBF em `v0.3.1`, pessoas/vínculos em `v0.4.0` e a folha mensal em `v0.5.0`. A revisão de identidades está implementada localmente e aguarda validação completa e publicação. O cadastro público está desabilitado e o login é reservado ao painel administrativo.
